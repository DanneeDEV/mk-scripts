...
(() => {
  ...
  // ---------- BOOT (manifest + snapshot) ----------
  const BASE = "https://rjndhhdwmmonsrniunhc.supabase.co/storage/v1/object/public/mk-snapshots";
  ...
  const parsePathParts = () =>
    location.pathname.replace(/^\/+|\/+$/g, "").split("/").filter(Boolean);

  if (!window.__MK_CAT_BOOT) {
    window.__MK_CAT_BOOT = (async () => {
      const manRes = await fetch(`${BASE}/current/manifest.json`, { cache: "no-store" });
      if (!manRes.ok) throw new Error("manifest http " + manRes.status);
      const manifest = await manRes.json();

      const mains = manifest?.mains || {};
      const mainKeys = new Set(Object.keys(mains));

      const parts = parsePathParts();
      const p0 = parts[0] || "";
      const p1 = parts[1] || "";

      const isCategoryPrefix = (p) => {
        const x = (p || "").toLowerCase();
        return x === "category" || x === "kategori";
      };

      const candidates = [
        p0.toLowerCase(),
        stripPrefixes(p0),
        isCategoryPrefix(p0) ? p1.toLowerCase() : null,
        isCategoryPrefix(p0) ? stripPrefixes(p1) : null,
      ].filter(Boolean);

      let mainSlug = candidates.find((c) => mainKeys.has(c)) || null;
      if (!mainSlug) {
        if (mainKeys.has("entreprenad")) mainSlug = "entreprenad";
        else throw new Error("Could not resolve mainSlug from path: " + location.pathname);
      }

      // subSlug if URL contains it
      let subSlug = null;
      if (isCategoryPrefix(p0)) {
        subSlug = parts[2] ? parts[2].toLowerCase() : null;
      } else {
        subSlug = (stripPrefixes(p0) === mainSlug) ? (parts[1] ? parts[1].toLowerCase() : null) : null;
      }

      const entry = mains[mainSlug];
      if (!entry?.url) throw new Error(`No manifest entry for mainSlug '${mainSlug}'`);

      const snapRes = await fetch(`${BASE}/${entry.url}`, { cache: "force-cache" });
      ...
      return {
        manifest,
        mainSlug,
        mainName: entry.name || prettyFromSlug(mainSlug),
        subSlug,
        subName: prettyFromSlug(subSlug),
        snapshot,
      };
    })();
  }
  ...
  (async () => {
    try {
      ...
      window.__MK_FADE?.readyOne?.(".mk-cat-stats[data-mk-fade]");
      ...
      console.log("[MK] all modules ready");

    } catch (e) {
      console.error("[MK] failed:", e);
    }
  })();
})();
